...
import type { QueuedMessage } from "@aevryn/ui/components/ui/input-message";
...
export function QueuedStack({
	...
	}, [stackCount, stackExpanded, stackBump.start, stackBump.set]);
							...
							{queue.map((item, i) => {
								...
								return (
									<motion.div
										...
										className={`group/qm absolute right-10 bottom-0 left-10 flex select-none items-center bg-[color-mix(in_oklab,var(--accent),var(--background)_68%)] ${
											compactStep
												? `gap-1.5 ${item.files.length > 0 ? "pl-1.5" : "pl-3"} pr-1`
												: `gap-2 ${item.files.length > 0 ? "pl-2" : "pl-3.5"} pr-1.5`
										} text-muted-foreground text-subtitle shadow-surface-3 active:cursor-grabbing ${shape.bg}`}
									>
										{item.files.length > 0 && (
											<div className="pointer-events-none flex shrink-0 items-center gap-1">
												...
												{item.files.length > 3 && (
													<span
														className={`flex ${compactStep ? "h-6 w-6" : "h-7 w-7"} items-center justify-center rounded-md bg-background/40 font-medium text-[11px] text-foreground/80 tabular-nums`}
													>
														+{item.files.length - 3}
													</span>
												)}
