...
} from "@aevryn/ui/components/fluid-hover-highlight";
import {
	Dialog,
	DialogContent,
	DialogDescription,
...
import {
	TabsSubtle,
	TabsSubtleItem,
} from "@aevryn/ui/components/ui/tabs-subtle";
import {
	type UseFluidHoverReturn,
	useFluidHover,
	useRegisterFluidHoverItem,
} from "@aevryn/ui/hooks/use-fluid-hover";
import { type IconComponent, useIcon } from "@aevryn/ui/lib/icon-context";
import { isDisabledRow } from "@aevryn/ui/lib/popup";
...
export function matchesShortcut(
	...
	const byCode =
		...
		e.code.toLowerCase() ===
			(/[a-z]/.test(parsed.key) ? `key${parsed.key}` : `digit${parsed.key}`);
...
export function sectionRows(
	...
	for (const item of visible) {
		...
		push(
			item.group ?? null,
			item.group ? `group:${item.group}` : "ungrouped",
			item,
		);
	}
...
const CommandMenuDialogContext =
	createContext<CommandMenuDialogContextValue | null>(null);
...
const CommandMenu = forwardRef<HTMLDivElement, CommandMenuProps>(
	(
		...
		const sections = useMemo(() => {
			const visible =
				query === "" ? items : items.filter((item) => filter(item, query));
			...
			(to: 1 | -1 | "first" | "last") => {
				...
				if (to === "first") next = enabled[0] ?? 0;
					...
					if (pos === -1)
						...
						next = enabled[(pos + to + enabled.length) % enabled.length] ?? 0;
				}
				setActiveIndex(next);
		...
		const handleKeyDown = (e: ReactKeyboardEvent<HTMLInputElement>) => {
			...
			switch (e.key) {
				...
				case "ArrowRight": {
					...
					const current = tabs.tabs.findIndex(
						(tab) => tab.value === tabs.value,
					);
			...
			>
				...
				<input
					...
					aria-activedescendant={
						activeIndex === null ? undefined : `${listId}-${activeIndex}`
					}
					autoComplete="off"
					autoCorrect="off"
					spellCheck={false}
...
export interface CommandMenuTabsProps
	extends Omit<HTMLAttributes<HTMLDivElement>, "onSelect"> {
	tabs: readonly CommandMenuTab[];
	value: string;
	onValueChange: (value: string) => void;
...
const CommandMenuList = forwardRef<HTMLDivElement, CommandMenuListProps>(
	({ className, children, renderItem, ...props }, ref) => {
		...
		return (
			...
			<ScrollArea
				className="scroll-divider [&::before]:!-top-px flex min-h-0 flex-1 flex-col border-border/60 border-t"
			...
			return (
				<div
					...
					className={cn(
						...
						"relative z-10 flex shrink-0 cursor-pointer select-none items-center outline-none",
						sizeClasses.control,
						sizeClasses.gap,
						sizeClasses.itemPx,
						sizeClasses.text,
                ...
                contrast step below it, on both states of the row. */}
							{item.description && (
								<span
									className={cn(
										"-my-1 min-w-0 truncate py-1 transition-[color] duration-80 [text-box:trim-both_cap_alphabetic]",
										isActive
