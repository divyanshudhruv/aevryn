...
import { useSizeVariant, type SizeVariant } from "@aevryn/ui/lib/size-context";
...
function pixelToValue(
  ...
  if (stepValues) return stepValues[nearestStepIndex(raw, stepValues)] ?? min;
    ...
    (index: number) => {
      ...
      if (!isNaN(parsed)) {
        ...
        const snapped = stepValues
          ? stepValues[nearestStepIndex(clamped, stepValues)] ?? clamped
  ...
  (
    ...
    const computeHoverPreview = useCallback(
      (cursorX: number, trackWidth: number) => {
        ...
        const rawVal = usable > 0 ? (clampedPx / usable) * (maxValue - minValue) + minValue : minValue;
        const snappedVal =
          stepValues && stepValues.length > 0
            ? stepValues[nearestStepIndex(rawVal, stepValues)] ?? rawVal
            : Math.max(
                minValue,
                Math.min(
                  maxValue,
                  Math.round((rawVal - minValue) / step) * step + minValue,
                ),
              );
        const snappedPercent = maxValue === minValue ? 0 : (snappedVal - minValue) / (maxValue - minValue);
    ...
    useLayoutEffect(() => {
      ...
      const px0 = valueToPixel(values[0] as number, min as number, max as number, w);
      motionX0.set(px0);
      if (isRange && values[1] !== undefined) {
        const px1 = valueToPixel(values[1] as number, min as number, max as number, w);
        motionX1.set(px1);
      }
    ...
    useEffect(() => {
      ...
      const ro = new ResizeObserver(([entry]) => {
        ...
        if (!dragging.current && initialSyncDone.current) {
          ...
          const px0 = valueToPixel(v[0] as number, mn as number, mx as number, w);
          animate(motionX0, px0, spring.moderate);
          if (isRange && v[1] !== undefined) {
            const px1 = valueToPixel(v[1] as number, mn as number, mx as number, w);
    ...
    useEffect(() => {
      ...
      if (tw <= 0) return;
      const v = valuesRef.current;
      const px0 = valueToPixel(v[0] as number, min as number, max as number, tw);
      animate(motionX0, px0, spring.moderate);
      if (isRange && v[1] !== undefined) {
        const px1 = valueToPixel(v[1] as number, min as number, max as number, tw);
        animate(motionX1, px1, spring.moderate);
      }
    ...
    const handlePointerDown = useCallback(
      (e: React.PointerEvent<HTMLDivElement>) => {
        ...
        const snappedPx = valueToPixel(snappedValue, (min as number), (max as number), layoutWidth);
    ...
    const handlePointerUp = useCallback(() => {
      ...
      const snapped = pixelToValue(currentPx, (min as number), (max as number), step, tw, stepValues);
      const snappedPx = valueToPixel(snapped, (min as number), (max as number), tw);
          ...
          ? stepValues
            ? stepValues.map((v) => ({
                ...
                percent:
                  max === min
                    ? 0
                    : (v - (min as number)) / ((max as number) - (min as number)),
              }))
            : Array.from(
                {
                  length:
                    Math.round(((max as number) - (min as number)) / step) + 1,
                },
                (_, i) => {
                  const v = (min as number) + i * step;
                  const percent =
                    (v - (min as number)) / ((max as number) - (min as number));
          ...
          >
            <SliderPrimitive.Control className="w-full h-full">
              ...
              <SliderPrimitive.Thumb
                ...
                getAriaValueText={
                  stepValues
                    ? () => formatValue(values[0] as number)
                    : undefined
                }
              ...
              {isRange && (
                <SliderPrimitive.Thumb
                  ...
                  getAriaValueText={
                    stepValues
                      ? () => formatValue(values[1] as number)
                      : undefined
                  }
                  className="block outline-none"
                  style={{ width: THUMB_SIZE, height: THUMB_SIZE }}
...
const ComfortableSlider = forwardRef<HTMLDivElement, SliderComfortableProps>(    ({
    ...
    const handlePointerDown = useCallback(
      (e: React.PointerEvent<HTMLDivElement>) => {
        ...
        dragging.current = true;
        setIsPressed(true);
        const newVal = getValueFromX(e.clientX)?? min;
    ...
    const handleRootPointerMove = useCallback(
      ...
      [handlePointerMove, onPointerMoveProp]
    );
    ...
    const handleRootPointerCancel = useCallback(
      ...
      [handlePointerUp, onPointerCancelProp]
    );
      ...
      >
        ...
        {variant === "pips" && (
          <div className="absolute inset-0 flex items-center px-2 z-[2] pointer-events-none" aria-hidden>
            {label && (
              <span className="text-[13px] px-2 text-transparent select-none">
                {label}
              </span>
            )}
            <span
              className="text-[13px] tabular-nums ml-auto px-2  text-transparent select-none"
