...
import { type SizeVariant, useSizeVariant } from "@aevryn/ui/lib/size-context";
	...
	(
		...
		const computeHoverPreview = useCallback(
			(cursorX: number, trackWidth: number) => {
				...
				const rawVal =
					usable > 0
						? (clampedPx / usable) * (maxValue - minValue) + minValue
						: minValue;
				const snappedVal =
			...
			>
				...
				>
					...
					{variant === "pips" && (
						...
						>
							{label && (
								<span className="select-none px-2 text-[13px] text-transparent">
									{label}
								</span>
							)}
							<span
								className="ml-auto select-none px-2 text-[13px] text-transparent tabular-nums"
