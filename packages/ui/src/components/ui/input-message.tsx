...
import { SurfaceProvider } from "@aevryn/ui/lib/surface-context";
			...
			<Tooltip content="Remove" side="top">
				<button
					...
					className={cn(
						...
						"cursor-pointer outline-none transition-opacity duration-80",
						"focus-visible:ring-1 focus-visible:ring-[color:var(--focus-ring,#6B97FF)]",
					)}
				>
	...
	(
		...
		useEffect(() => {
			...
			if (prev === "streaming" && status === "idle" && queueArr.length > 0) {
				const head = queueArr[0];
				if (head == null) return;
				const rest = queueArr.slice(1);
				onQueueChange?.(rest);
		...
		const moveQueued = useCallback(
			(item: QueuedMessage, dir: -1 | 1) => {
				...
				const j = i + dir;
				const a = i >= 0 ? cur[i] : undefined;
				const b = j >= 0 && j < cur.length ? cur[j] : undefined;
				...
				if (
					...
					if (activeSuggestion != null) {
						...
						if (e.key === "Enter") {
							...
							if (picked != null) acceptSuggestion(picked);
							return;
						}
						if (e.key === "Escape") {
							e.preventDefault();
