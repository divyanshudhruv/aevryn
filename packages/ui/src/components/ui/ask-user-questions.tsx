...
const AskUserQuestions = forwardRef<HTMLDivElement, AskUserQuestionsProps>(
	function AskUserQuestions(
		...
		const moveActive = useCallback(
			...
			[allowOther, otherIndex, setActiveIndex, focusRow],
		);
					...
					{(() => {
						...
						return (
							<motion.div
								...
								aria-hidden
								className={cn(
									"pointer-events-none absolute bg-card ring-1 ring-border ring-inset",
									shape.bg,
								)}
								initial={{
									opacity: 0,
										...
										<Field.Control
											...
											render={
												<textarea
													...
													className={cn(
														"m-0 block w-full resize-none overflow-hidden border-0 bg-transparent p-0 text-foreground leading-snug outline-none placeholder:text-muted-foreground",
														sizeClasses.text,
													)}
													style={{ fontVariationSettings: fontWeights.medium }}
												/>
              ...
              in lockstep. */}
					{showFooter && (
						<div
							className={cn(
								"pt-1",
								compact ? "px-3.5 pb-1.5 sm:px-4" : "px-4 pb-2 sm:px-5",
							)}
						>
										...
										<Field.Error
											...
											render={
												<motion.p
													...
													className="min-w-0 px-2 text-left text-[12px] text-destructive leading-snug sm:px-3"
												/>
											}
										>
											{freeTextError}
...
function ShortcutChip({
	...
	return (
		<kbd
			...
			className={cn(
				"inline-flex h-[18px] min-w-[18px] items-center justify-center gap-0.5 px-1 font-sans text-[11px] leading-none tracking-wide",
				tone === "inverted"
