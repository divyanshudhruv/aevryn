	...
	(
		...
		const actionElement =
			variant === "button" ? (
				<span
					className={cn(
						"flex shrink-0 items-center gap-1.5 px-1.5 transition-colors duration-80",
						rowPy,
						sizeClasses.text,
					...
					<AnimatePresence mode="wait" initial={false}>
						{status === "error" ? (
							...
							>
								...
								<span className="inline-grid select-none text-left">
									<span
		...
		const button = (
			<button
				...
				className={cn(
					"group flex w-full cursor-pointer items-center outline-none transition-all duration-80",
