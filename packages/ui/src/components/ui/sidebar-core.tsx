...
import { Button, type ButtonProps } from "@aevryn/ui/components/ui/button";
...
import { useSize, useSizeVariant } from "@aevryn/ui/lib/size-context";
...
const SidebarTrigger = forwardRef<HTMLButtonElement, SidebarTriggerProps>(
	({ onClick, size, children, ...props }, ref) => {
		...
		const iconSize =
			useSizeVariant() === "compact"
				? ("icon-compact" as const)
				: ("icon" as const);
		...
		return (
			<main
				...
				className={cn(
					...
					"peer-data-[variant=inset]:peer-data-[side=right]:mr-0 peer-data-[variant=inset]:peer-data-[side=left]:ml-0 peer-data-[variant=inset]:m-2",
					"peer-data-[variant=inset]:peer-data-[state=collapsed]:peer-data-[side=right]:mr-2 peer-data-[variant=inset]:peer-data-[state=collapsed]:peer-data-[side=left]:ml-2",
				...
				<>
					...
					<span
						className={cn(
							...
							group.open
								? "pointer-coarse:w-6 w-0 opacity-0 pointer-coarse:opacity-100 group-focus-within/group-header:w-6 group-focus-within/group-header:opacity-100 group-hover/group-header:w-6 group-hover/group-header:opacity-100 group-has-[[data-sidebar=group-action]:is([data-state=open],[data-popup-open],[aria-expanded=true])]/group-header:w-6 group-has-[[data-sidebar=group-action]:is([data-state=open],[data-popup-open],[aria-expanded=true])]/group-header:opacity-100"
								: "w-6 opacity-100",
						)}
					>
						<motion.span
...
>(({ className, children, ...props }, ref) => {
	return (
		<div
			...
			className={cn(
				"absolute top-2 right-3.5 z-10 flex h-8 items-center gap-1",
				className,
			)}
			{...props}
		>
