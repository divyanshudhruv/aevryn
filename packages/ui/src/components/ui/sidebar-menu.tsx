...
export interface SidebarMenuButtonProps
  ...
  size?: "default" | "sm" | "lg";
  icon?: IconComponent;
  /** Execution status: drives the leading indicator (`running` → dotmatrix
   *  spinner, `failed` → triangle, `sleeping`/`waiting`/`awaiting_approval`
   *  → circular dotmatrix, `completed` → filled dot, others → hollow ring)
   *  and stamps `data-status`. */
            ...
            {status === "running" ? (
              ...
              <span
                className={cn(
                  ...
                  lit ? "border-foreground/60" : "border-muted-foreground/50",
                )}
              />
            )}
          </span>
