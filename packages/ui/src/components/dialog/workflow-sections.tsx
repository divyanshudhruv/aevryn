...
function PlanPanel({ workflowId }: { workflowId: string }) {
  ...
  const persist = useCallback(
    async (steps: Array<{ id?: string; title: string; description: string }>) => {
  ...
  return (
    <div className="flex flex-col">
      ...
      <p className="mt-4 text-[12px] text-muted-foreground">
        Memories are scoped to this conversation. They are shared with the
        agent here and never cross to other threads.
      </p>
    </div>
  );
}
