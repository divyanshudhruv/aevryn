...
export function DotMatrixBase({
  ...
  const dots = Array.from({ length: MATRIX_SIZE * MATRIX_SIZE }).map((_, index) => {
    ...
    const dotStyle = {
      ...
      ...(isActive &&
      stylePatch != null &&
      typeof stylePatch.opacity === "number"
        ? { opacity: String(styleOpacity(stylePatch.opacity)) }
        : {}),
      ...(isActive && stylePatch != null
        ? (() => {
          const bloomLevel = (stylePatch as CSSProperties & { "--dmx-bloom-level"?: number })["--dmx-bloom-level"];
          return typeof bloomLevel === "number"
            ? { ["--dmx-bloom-level" as const]: String(bloomLevel) }
            : {};
        })()
        : {}),
      ...(!isActive
