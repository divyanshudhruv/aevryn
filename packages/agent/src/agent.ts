...
// Mid-stream provider-error retries. The SDK's stream retry buffers TOOL
// parts but streams TEXT live: on a retry it closes the first attempt's text
// part and the new attempt re-streams the same narration as a NEW text part —
// the user sees the turn duplicated in real time, and no persistence-side
// collapse can undo what was already rendered. With the deterministic loop
// guards (replayedStepGuardStop / attemptReplayGuardStop) and the repair
// path (experimental_repairToolCall) in place, retries are pure downside:
// transient blips now fail the turn loudly (error tile + failed status +
// retryAgent repair) instead of silently replaying content.
export const STREAM_RETRIES = 0;

/** Set true by the agent's stream callbacks while a stream retry is in
 *  flight; the service layer reads it in onEnd to surface a "retried"
 *  notice on the persisted turn. Reset per respond() invocation. */
...
export function createAevrynAgent(opts: AevrynAgentOptions) {
	...
	return new ToolLoopAgent({
		...
		// Separate secret from VAULT_KEY so crypto- and approval-key rotation
